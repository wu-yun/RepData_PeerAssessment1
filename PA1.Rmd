...
1. Load the data (assuming the current working directory is already set to the directory containing the data file)
```{r,echo=TRUE}
...
weekdayssub <- completedata[weekdays(completedata$date)!="Saturday" & weekdays(completedata$date)!="Sunday", c(1:3)]
weekdaysavg <- aggregate(weekdayssub$steps,by=list(weekdayssub$interval),mean)
colnames(weekdaysavg) <- c("interval", "steps")
weekendsub <-completedata[weekdays(completedata$date)=="Saturday" | weekdays(completedata$date)=="Sunday", c(1:3)]
weekendavg <-aggregate(weekendsub$steps,by=list(weekendsub$interval),mean)
colnames(weekendavg) <- c("interval", "steps")
ggplot(weekdaysavg,aes(interval,steps))+geom_line()+ggtitle("Weekday")
ggplot(weekendavg,aes(interval,steps))+geom_line()+ggtitle("Weekend")
```
